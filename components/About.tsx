...
const About: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-gray-50 transition-colors duration-300 relative overflow-hidden" id="about">
      <div 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          <div className={`reveal-item ${isRevealed ? 'revealed' : ''}`}>
            ...
            <h3 className="text-4xl font-bold text-jet mb-6">Built for the Bold. Driven by Results.</h3>
            ...
            <div className="space-y-6">
              <h4 className="font-bold text-xl border-b border-gray-200 pb-2 text-jet">Core Values</h4>
          ...
          <div className={`relative reveal-item ${isRevealed ? 'revealed' : ''}`} style={{ transitionDelay: '300ms' }}>
            <div 
              className="absolute -z-10 top-0 left-0 w-full h-full bg-white/50 backdrop-blur-sm rounded-3xl border border-white/20 transition-transform duration-75 ease-out"
              style={{ transform: `rotate(${(scrollY - 500) * 0.01}deg)` }}
            ></div>
            
            <h4 className="font-bold text-xl mb-6 pl-4 text-jet">Industries Served</h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {INDUSTRIES.map((ind, idx) => (
                <div 
                  key={ind.label} 
                  className={`bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-500 flex flex-col items-center text-center gap-3 border border-gray-100 group`}
                ...
                >
                  ...
                  <span className="font-bold text-sm text-jet">{ind.label}</span>
                </div>
              ))}
            </div>
          </div>
